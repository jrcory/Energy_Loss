...
//get anode anode open outfile 
    ...
    double scales[20]={1.17284,1.1373,1.1283,1.11533,1.12736,1.11893,1.14578,1.10413,1.11586,1.14767,1.09294,1.13117,1.1231,1.12486,1.15222,1.12925,1.11546,1.12086,1.1455,1.02635};
...
//initlize basic gas properties for methane run 
    ...
    methane.density(8.7759e-5);
    methane.thickness(8.7759e-5*0.25);
   //CHANGED TO 1/5 THE THICKNESS
...
//cant add the five over thing because we are utilizing a varying length here
        cout << "mult: " << mult << endl;
	for (int k=0;k<mult;k++){
	   ...
	   methane.thickness(8.7759e-5*hf);
	   float Etemp; 
           for (int i=anode;i<20;i++){
	     auto result = catima::calculate(ER,methane,E_out[i]);  
	     Etemp = ran.Gaus(result.Eloss,0.1/2.355)*scales[i]; 
       	     Eloss[i]+= Etemp;
